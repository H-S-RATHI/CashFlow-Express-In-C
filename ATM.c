...
// Function prototypes
int authenticateUser(BankAccount *accounts, int numAccounts, int enteredAccountNumber, int enteredPin);
...
void performTransactions(BankAccount *accounts, int loggedInAccountIndex, int numAccounts);
            ...
            case 1:
                {
                    ...
                    int loggedInAccountIndex = authenticateUser(accounts, numAccounts, enteredAccountNumber, enteredPin);
...
int authenticateUser(BankAccount *accounts, int numAccounts, int enteredAccountNumber, int enteredPin) {
    ...
    for (int i = 0; i < numAccounts; i++) {
        if (accounts[i].accountNumber == enteredAccountNumber && accounts[i].pin == enteredPin) {
...
void openAccount(BankAccount *accounts, int *numAccounts) {
    if (*numAccounts < 10) {
        ...
        newAccount.accountNumber = 1000 + *numAccounts; // Example way of generating account numbers
        ...
        switch (option) {
            case 1:
                ...
                if (amount > 0 && amount <= accounts[loggedInAccountIndex].balance) {
                    ...
                    saveAccountsToFile(accounts, numAccounts); // Update and save account data
                } else {
...
void saveAccountsToFile(BankAccount *accounts, int numAccounts) {
    ...
    for (int i = 0; i < numAccounts; i++) {
        fprintf(file, "%d %d %" PRId64 " %s %s %s\n", accounts[i].accountNumber, accounts[i].pin,
                accounts[i].balance, accounts[i].name, accounts[i].email, accounts[i].phone);
    }
...
void loadAccountsFromFile(BankAccount *accounts, int *numAccounts) {
    ...
    *numAccounts = 0;
    while (*numAccounts < 10 && fscanf(file, "%d %d %" PRId64 " %s %s %s",
                                       &accounts[*numAccounts].accountNumber, &accounts[*numAccounts].pin,
                                       &accounts[*numAccounts].balance, accounts[*numAccounts].name,
                                       accounts[*numAccounts].email, accounts[*numAccounts].phone) != EOF) {
        (*numAccounts)++;
    }
